...
Quick and easy AI components to build Chatbot
...
```shell
pip install chatbot@git+ssh://git@github.com/Cinnamon/chatbot.git
```
...
purposes only. Thus, it should not be used in the main source code (i.e. `chatbot/` and `tests/`), but can be used in `examples/`.
