...
def export(export_path, output):
    ...
    from chatbot.contribs.promptui.config import export_pipeline_to_config
...
def run(run_path, share, username, password, appname, port):
    ...
    from chatbot.contribs.promptui.ui import build_from_dict
    ...
    if share:
        ...
        if appname:
            from chatbot.contribs.promptui.tunnel import Tunnel
...
def makedoc(module, output, separation_level):
    ...
    Example:
        ...
        \b
        # Make component documentation for chatbot library
        $ kh makedoc chatbot
    """
    from chatbot.contribs.docs import make_doc
...
def start_project(template):
    ...
    which is located at https://github.com/Cinnamon/chatbot/tree/main/templates
    ...
    os.system(
        "cookiecutter git@github.com:Cinnamon/chatbot.git "
