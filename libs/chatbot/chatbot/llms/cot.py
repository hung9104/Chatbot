...
from chatbot.base import BaseComponent, Document
...
class Thought(BaseComponent):
    ...
    ```python
    >> from chatbot.pipelines.cot import Thought
    >> thought = Thought(
...
class ManualSequentialChainOfThought(BaseComponent):
    ...
    `chatbot.pipelines.cot.Thought`. Please refer that section for
    ...
    **Create and run a chain of thought without "+" operator:**
    ...
    ```pycon
    >>> from chatbot.pipelines.cot import Thought, ManualSequentialChainOfThought
    >>> llm = LCAzureChatOpenAI(...)
    >>> thought1 = Thought(
    >>>    prompt="Word {word} in {language} is ",
    >>>    post_process=lambda string: {"translated": string},
    ...
    **Create and run a chain of thought without "+" operator:** Please refer the
    `chatbot.pipelines.cot.Thought` section for examples.
    ...
    llm: LLM = Param(help="The LLM model to use (base of chatbot.llms.BaseLLM)")
