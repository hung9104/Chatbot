...
function install_dependencies() {
    # check if the env is already setup by finding 'chatbot' in 'pip list'
    if pip list 2>/dev/null | grep -q "chatbot"; then
        echo "Requirements are already installed"
    else
        local chatbot_root="$(pwd)/libs/chatbot"
        local chatbot_app_root="$(pwd)/libs/chatbot_app/"
        ...
        if [ -f "pyproject.toml" ]; then
            ...
            echo "" && echo "Installing libs/chatbot"
            python -m pip install -e "$chatbot_root"
            echo "" && echo "Installing libs/chatbot_app"
            python -m pip install -e "$chatbot_app_root"
...
target_pdf_js_dir="$(pwd)/libs/chatbot_app/chatbot_app/assets/prebuilt/${pdf_js_dist_name}"
...
print_highlight "Launching Chatbot in your browser, please wait..."
