...
CALL :print_highlight "Updating Chatbot to latest"
CALL :update_latest
IF ERRORLEVEL 1 GOTO :end
...
:update_latest
FOR /F "tokens=1,2" %%a in ('pip list') do if "%%a"=="chatbot-app" set current_version=%%b
...
FOR /F "tokens=1,2" %%a in ('pip list') do if "%%a"=="chatbot-app" set updated_version=%%b
