...
CALL :print_highlight "Installing Chatbot"
...
CALL :print_highlight "Launching Chatbot in your browser, please wait..."
CALL :launch_ui
...
pip list | findstr /C:"chatbot" >NUL 2>&1
IF %ERRORLEVEL% == 0  (
    ...
    IF EXIST "pyproject.toml" (
        ...
        ECHO Installing libs\chatbot
        python -m pip install -e "%CD%\libs\chatbot"

        ECHO Installing libs\chatbot_app
        python -m pip install -e "%CD%\libs\chatbot_app"

        python -m pip install --no-deps -e .
    ) ELSE (
        ECHO Installing Chatbot %app_version%
        @REM Work around for versioning control
        python -m pip install git+https://github.com/Cinnamon/chatbot.git@"%app_version%"#subdirectory=libs/chatbot
        python -m pip install git+https://github.com/Cinnamon/chatbot.git@"%app_version%"#subdirectory=libs/chatbot_app
        python -m pip install --no-deps git+https://github.com/Cinnamon/chatbot.git@"%app_version%"
    )

    ( CALL pip list | findstr /C:"chatbot" >NUL 2>&1 ) || (
...
set "target_pdf_js_dir=%current_dir%\libs\chatbot_app\chatbot_app\assets\prebuilt\%pdf_js_dist_name%"
...
ECHO Starting Chatbot UI... (prebuilt PDF.js is at %PDFJS_PREBUILT_DIR%)
