...
- Clone the repo
  ...
  ```shell
  git clone git@github.com:Cinnamon/chatbot.git
  cd chatbot
  ```
...
`chatbot` library focuses on the AI building blocks to implement a RAG-based QA application. It consists of base interfaces, core components and a list of utilities:

- Base interfaces: `chatbot` defines the base interface of a component in a pipeline. A pipeline is also a component. By clearly define this interface, a pipeline of steps can be easily constructed and orchestrated.
- Core components: `chatbot` implements (or wraps 3rd-party libraries
  like Langchain, llama-index,... when possible) commonly used components in
  chatbot use cases. Some of these components are: LLM, vector store,
...
- List of utilities: `chatbot` provides utilities and tools that are
