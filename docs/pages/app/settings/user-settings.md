...
`chatbot_app` allows developers to extend the index and the reasoning pipeline. In
...
`chatbot_app` allows developers to declare such user settings in their code. Once
declared, `chatbot_app` will render them in a Settings page.

There are 2 places that `chatbot_app` looks for declared user settings. You can
