...
In essence, a pipeline will subclass from `chatbot.base.BaseComponent`.
...
```python
from chatbot.base import BaseComponent
...
```python
    def run(self, question: str, history: list, **kwargs) -> Document:
        ...
        Returns:
            chatbot.base.Document: the final answer
        """
```

## Register your pipeline to chatbot_app

To register your pipelines to chatbot_app, you declare it in the `flowsettings.py`
file. This file locates at the current working directory where you start the
chatbot_app. In most use cases, it is this
[one](https://github.com/Cinnamon/chatbot/blob/main/flowsettings.py).
...
You can register multiple reasoning pipelines to chatbot_app by populating the
...
allow the users to configure as a dictionary. `chatbot_app` will include them into the
...
Once adding these methods to your pipeline class, `chatbot_app` will automatically
extract and add them to the settings.

## Construct to pipeline object

Once `chatbot_app` runs your pipeline, it will call your classmethod `get_pipeline`
...
```python
from chatbot_app.embeddings.manager import embeddings
from chatbot_app.llms.manager import llms
...
from chatbot_app.db.models import Source, Index, Conversation, User
