...
The chatbot_app has default indexing pipeline: `chatbot_app.index.file.pipelines.IndexDocumentPipeline`.
...
- `get_pipeline(cls, user_settings, index_settings)`: return the
  fully-initialized pipeline, ready to be used by chatbot_app.
  - `user_settings`: is a dictionary contains user settings (e.g. `{"pdf_mode": True, "num_retrieval": 5}`). You can declare these settings in the `get_user_settings` classmethod. chatbot_app will collect these settings into the app Settings page, and will supply these user settings to your `get_pipeline` method.
...
The chatbot_app has default retrieval pipeline:
`chatbot_app.index.file.pipelines.DocumentRetrievalPipeline`. This pipeline works as
follow:
...
| Vector store     | self.\_VS     | - self.\_VS.add: add the list of embeddings to the vector store (optionally associate metadata and ids)<br>- self.\_VS.delete: delete vector entries based on ids<br>- self.\_VS.query: get embeddings based on embeddings.                                                                 | chatbot > storages > vectorstores > BaseVectorStore       |
| Doc store        | self.\_DS     | - self.\_DS.add: add the segments to document stores<br>- self.\_DS.get: get the segments based on id<br>- self.\_DS.get_all: get all segments<br>- self.\_DS.delete: delete segments based on id                                                                                           | chatbot > storages > docstores > base > BaseDocumentStore |
